...
(defn ^:private execute-loaded-rule* [forms requires entry]
  (let [check-fn (:check-fn entry)
    ...
    (if-not (nil? check-fn) (let [results (execute-check-fn-in-forms forms requires check-fn) results-with-metadata (add-match-code-as-meta results)] (-> entry (assoc :result (-> results seq boolean condition-fn)) (assoc :findings results-with-metadata))) entry)))
